  ...
  if (!game) {
    els.theaterPanel.innerHTML = `
      <div class="empty-player theater-empty">
        <span class="play-icon"></span>
        <h3>Select a match</h3>
        <p>Open any fixture or live card to launch a wide theater player with streams and match context.</p>
      </div>
    `;
    return;
  }
        ...
        ${
          activeStream
            ...
            : `<div class="player-placeholder">
                <div>
                  <span class="play-icon"></span>
                  <h3>${status === "loading" ? "Finding live sources" : "Theater waiting"}</h3>
                  <p>${playerMessage(status, sourceMatch)}</p>
                </div>
              </div>`
        }
